...
exports.getAllKomoditi = (lastDate, searchDate, cb) => {
  ...
  db.query(`SELECT price.id, komoditi_kat.komoditi_name, satuan, tanggal, p_guntur, p_kadungora, p_cikajang, p_pamengpeuk, p_samarang, p_malangbong, med_minggu_ini, med_minggu_lalu, keterangan, persentase FROM price INNER JOIN komoditi_kat ON price.id_komoditi = komoditi_kat.id WHERE price.tanggal LIKE '%${setDate}%'`, (err, res) => {
    if(err) {
      throw err;
    }
    cb(err, res.rows);
...
exports.komoditiListNaikTurun = (lastDate, searchDate, cb) => {
  const setDate = searchDate === undefined ? lastDate : searchDate
  db.query(`SELECT price.id, komoditi_kat.komoditi_name, satuan, tanggal, p_guntur, p_kadungora, p_cikajang, p_pamengpeuk, p_samarang, p_malangbong, med_minggu_ini, med_minggu_lalu, keterangan, persentase FROM price INNER JOIN komoditi_kat ON price.id_komoditi = komoditi_kat.id WHERE price.tanggal LIKE '%${setDate}%'  AND (keterangan = 'Naik' OR keterangan = 'Turun')`, (err, res) => {
    if(err) {
      throw err;
    }
    cb(err, res.rows);
...
exports.komoditiListNaik = (lastDate, searchDate, cb) => {
  const setDate = searchDate === undefined ? lastDate : searchDate
  db.query(`SELECT keterangan FROM price WHERE price.tanggal LIKE '%${setDate}%' AND keterangan = 'Naik'`, (err, res) => {
    if(err) {
      throw err;
    }
    cb(err, res);
...
exports.komoditiListTurun = (lastDate, searchDate, cb) => {
  const setDate = searchDate === undefined ? lastDate : searchDate
  db.query(`SELECT keterangan FROM price WHERE price.tanggal LIKE '%${setDate}%' AND keterangan = 'Turun'`, (err, res) => {
    if(err) {
      throw err;
    }
    cb(err, res);
...
exports.komoditiListTetap = (lastDate, searchDate, cb) => {
  const setDate = searchDate === undefined ? lastDate : searchDate
  db.query(`SELECT keterangan FROM price WHERE price.tanggal LIKE '%${setDate}%' AND keterangan = 'Tetap'`, (err, res) => {
    if(err) {
      throw err;
    }
    cb(err, res);
